...
La \textit{Digital Twin Interface} \`e la componente web attraverso cui l'utente interagisce con il Gemello Digitale. La sua posizione nell'architettura \`e determinante: riceve le intenzioni dell'utente, rende visibili informazioni provenienti dai moduli interni e restituisce un riscontro prima che un comando o un'automazione raggiungano l'ambiente domestico fisico.

L'interfaccia \`e organizzata in quattro aree principali:
\begin{itemize}
    \item \textbf{Dashboard}, che riassume lo stato della casa, i dispositivi e i principali indicatori energetici;
    \item \textbf{Consumption}, che espone grafici storici e predittivi alimentati dal \textit{Data Analysis Module};
    \item \textbf{Automations}, che mostra le routine sincronizzate con \textit{Home Assistant}, il loro stato e i suggerimenti derivati dalla simulazione;
    \item \textbf{Configuration e User area}, che consentono di configurare casa, dispositivi, piano energetico e preferenze utente.
\end{itemize}

La \textit{Digital Twin Interface} comunica con il \textit{Simulation Management Module} per sottoporre comandi e regole a una verifica preventiva; dopo una validazione positiva, il \textit{Home Assistant Integration Module} propaga le operazioni verso \textit{Home Assistant}. Nella direzione opposta, stati dei dispositivi, consumi e automazioni esistenti sono sincronizzati verso l'interfaccia, cos\`i che l'utente operi su una rappresentazione aggiornata della smart home.

Il sistema di partenza permette soprattutto di consultare e gestire automazioni gi\`a presenti. Il contributo della tesi estende la sezione \textit{Automations} con un \textit{builder} visuale che rende espliciti trigger, condizioni e azioni, mantenendo nel medesimo flusso la simulazione e il salvataggio della regola. Un canale conversazionale pu\`o rimanere complementare, ma la GUI consente di verificare con maggiore chiarezza il risultato della composizione.

Questa sezione chiarisce quindi il ruolo dell'interfaccia prima di introdurre, nel Capitolo~4, il dettaglio del progetto e dell'implementazione del nuovo flusso TAP.
