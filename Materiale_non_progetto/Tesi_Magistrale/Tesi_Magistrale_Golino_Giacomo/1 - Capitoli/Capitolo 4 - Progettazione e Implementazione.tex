...
La versione finale dell'interfaccia \`e stata aggiornata per operare direttamente sui dati provenienti dall'istanza reale di Home Assistant utilizzata nel caso di studio. A partire dal backup dell'abitazione intelligente, il sistema importa dispositivi, entit\`a e automazioni esistenti, rendendoli disponibili nel Gemello Digitale attraverso le API di integrazione. Questo aggiornamento consente di superare una logica puramente dimostrativa e di lavorare su un insieme di dispositivi effettivamente configurati, mantenendo la sincronizzazione tra rappresentazione digitale e stato della Smart Home.

Un primo intervento ha riguardato la configurazione della planimetria e dei dispositivi. La mappa del primo piano \`e stata caricata come immagine di riferimento e viene utilizzata nella sezione di configurazione per collocare gli oggetti domestici nello spazio fisico. La Figura~\ref{fig:cap4_impl_configuration_map} mostra la schermata implementata: sulla sinistra \`e visibile la planimetria dell'abitazione, mentre sulla destra \`e presente la lista dei dispositivi estratti dall'integrazione Home Assistant. Questa organizzazione permette di associare le entit\`a software alla loro posizione nello spazio domestico, rafforzando la coerenza del Gemello Digitale.
...
Il secondo intervento ha riguardato la creazione e gestione delle automazioni. La pagina di aggiunta delle regole TAP \`e stata resa bilingue, con etichette e messaggi in italiano o in inglese in base alla lingua selezionata dall'utente. Il salvataggio di una nuova automazione ora produce una configurazione compatibile con Home Assistant e, dopo la creazione, la regola risulta visibile nella pagina di elenco delle automazioni. Anche la cancellazione \`e stata resa pi\`u sicura: prima dell'eliminazione viene richiesta una conferma esplicita e, in caso di esito positivo, il sistema restituisce un messaggio informativo non ambiguo.

Particolare attenzione \`e stata dedicata al robot aspirapolvere. L'esposizione diretta degli script Home Assistant avrebbe obbligato l'utente a scegliere comandi tecnici come \texttt{script.qrevo\_soggiorno\_aspira}. Per ridurre il carico cognitivo, l'interfaccia aggrega tali script in un controllo orientato al dominio: l'utente sceglie prima il dispositivo \emph{Qrevo Curv 2 Pro}, poi il tipo di intervento, tra \emph{Aspira}, \emph{Lava} e \emph{Aspira e lava}, e infine una o pi\`u stanze. Internamente il sistema traduce questa scelta in una o pi\`u chiamate \texttt{script.turn\_on}, una per ogni stanza selezionata, preservando la compatibilit\`a con Home Assistant ma mostrando all'utente un modello mentale pi\`u vicino all'azione domestica reale.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{images/Chap - 4/Implementation/implementation_automation_builder_robot.png}
    \caption{Creazione di un'automazione TAP con azione semantica sul robot: selezione della modalit\`a di pulizia e delle stanze.}
    \label{fig:cap4_impl_robot_automation}
\end{figure}

La stessa schermata \`e stata inoltre rivista dal punto di vista responsivo. Le righe dedicate a condizioni e azioni non utilizzano pi\`u una disposizione orizzontale rigida, ma una griglia capace di adattarsi alla larghezza disponibile. In questo modo le icone, i campi di selezione, i pulsanti di eliminazione e l'anteprima dell'automazione non si sovrappongono, anche quando vengono utilizzati nomi lunghi o azioni multi-stanza. Questo aspetto \`e rilevante perch\'e la costruzione delle automazioni richiede la lettura simultanea di pi\`u elementi: trigger, dispositivi, stati, azioni e risultato previsto.

Nel complesso, questi aggiornamenti rafforzano tre propriet\`a dell'interfaccia: la fedelt\`a rispetto allo stato della Smart Home reale, la comprensibilit\`a delle azioni complesse e la prevenzione degli errori operativi. L'utente non deve conoscere la struttura interna degli script Home Assistant, ma pu\`o comunque ottenere una configurazione tecnicamente corretta e verificabile dal sistema.
