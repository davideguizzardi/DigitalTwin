...
export const StyledButton = ({ variant = "primary",className="", children , disabled=false, ...props}) => {
  const theme = {
  ...
  const classSyle=`${theme[variant]} ${className}`
  return <Button {...props} className={classSyle} color="test" disabled={disabled}>{children}</Button>;
};
